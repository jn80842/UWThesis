...
rules, operating over the space of Halide expressions (see appendex~\ref{a:grammar} for
...
The Halide compiler contains a $\texttt{SolveExpression}$ class that implements functionality that we will refer to as a variable solver in this work. It takes an expression and a variable name and `solves' the expression for that variable, isolating it on the left of an expression. In the formalism described above, $\phi(s)$ is true if $s$ is in the form $v \circ e$, where $v$ is the target variable, $\circ$ is some binary operator, and $e$ is a term that does not contain the target variable $v$.
